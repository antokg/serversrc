...
void CHARACTER_MANAGER::Destroy()
{
	...
	while (it != m_map_pkChrByVID.end()) {
		...
		// TODO retourner l'iterator pour eviter de re parcourir la map
...
#endif
{
	...
	if (0 != ch->GetPlayerID())
	{
		const auto it = m_map_pkChrByPID.find(ch->GetPlayerID());
...
{
	for (const auto& c : m_set_pkChrMonsterLog)
	{
...
LPCHARACTER CHARACTER_MANAGER::FindSpecifyPC(unsigned int uiJobFlag, long lMapIndex, LPCHARACTER except, int iMinLevel, int iMaxLevel)
{
	...
	for (const auto& it : m_map_pkChrByPID)
	{
		LPCHARACTER ch = it.second;
