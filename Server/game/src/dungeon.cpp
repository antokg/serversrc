...
{
	const auto it =  m_map_Flag.find(name);
...
{
	const auto it = m_set_pkCharacter.find(ch);
...
{
	const auto it = m_map_pkDungeon.find(dungeon_id);
...
bool CDungeon::IsValidRegen(LPREGEN regen, size_t regen_id) {
	const auto it = std::find(m_regen.begin(), m_regen.end(), regen);
