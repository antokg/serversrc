...
bool ITEM_MANAGER::ReadMonsterDropItemGroup(const char * c_pszFileName)
{
	...
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		if (strType == "kill")
		{
			CMobItemGroup * pkGroup = M2_NEW CMobItemGroup(iKillDrop, stName);
		...
		else if (strType == "drop")
		{
			...
			bool bNew = true;
			const auto it = m_map_pkDropItemGroup.find (iMobVnum);
			if (it == m_map_pkDropItemGroup.end())
			{
				pkGroup = M2_NEW CDropItemGroup(stName);
			}
			else
			{
				bNew = false;
		...
		{
			CBuyerThiefGlovesItemGroup* pkGroup = M2_NEW CBuyerThiefGlovesItemGroup(stName);
