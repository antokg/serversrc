...
typedef struct SPlayerItem
{
	...
	DWORD	vnum;
	int	alSockets[ITEM_SOCKET_MAX_NUM];	// 소켓번호
...
typedef struct SItemTable : public SEntityTable
{
	...
	int        alValues[ITEM_VALUES_MAX_NUM];
	...
	unsigned char		cLimitRealTimeFirstUseIndex;		// 아이템 limit 필드값 중에서 LIMIT_REAL_TIME_FIRST_USE 플래그의 위치 (없으면 -1)
	unsigned char		cLimitTimerBasedOnWearIndex;		// 아이템 limit 필드값 중에서 LIMIT_TIMER_BASED_ON_WEAR 플래그의 위치 (없으면 -1) 
