...
namespace marriage
{
	...
	void CManager::Remove(DWORD dwPID1, DWORD dwPID2)
	{
		...
		if (!pMarriage || pMarriage->GetOther(dwPID1) != dwPID2)
		{
			for (const auto& it : m_MarriageByPID)
			{
	...
	{
		// 결혼한 사람들 보내기
		for (const auto& pMarriage : m_Marriages)
		{
			{
				TPacketMarriageAdd p;
		...
		// 결혼식 보내기
		for (const auto& it : m_mapRunningWedding)
		{
	...
	{
		const auto it = m_mapRunningWedding.find(make_pair(dwPID1, dwPID2));
	...
	void CManager::Update()
	{
		...
		if (!m_pqWeddingEnd.empty())
		{
			while (!m_pqWeddingEnd.empty() && m_pqWeddingEnd.top().dwTime <= now)
			{
				...
				const auto it = m_mapRunningWedding.find(make_pair(wi.dwPID1, wi.dwPID2));
				if (it == m_mapRunningWedding.end())
					continue;
				...
				const auto it_marriage = m_MarriageByPID.find(w.dwPID1);
